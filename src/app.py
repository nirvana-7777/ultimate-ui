...
webepg_client = WebEPGClient(
    base_url=config.get('webepg.url'),
    timeout=config.get('webepg.timeout', 10)
)
...
def epg_display():
    ...
    try:
        ...
        return render_template('epg_display.html',
...
def configuration():
    ...
    if request.method == 'POST':
        try:
            ...
            ultimate_backend_client.base_url = config_data['ultimate_backend']['url'].rstrip('/')
...
def epg_mapping():
    ...
    try:
        ...
        if ultimate_providers:
            ...
            ultimate_channels = ultimate_backend_client.get_provider_channels(selected_provider)

        return render_template('epg_mapping.html',
...
def api_create_alias():
    ...
    try:
        ...
        result = webepg_client.create_channel_alias(channel_identifier, alias, alias_type)
